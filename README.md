...
For chat, `--output json` returns a structured single-turn payload that is easy to consume from a VS Code extension. When chat is in JSON mode, the CLI writes a stderr notice so you know stdout is machine-readable. For longer prompts, `--prompt-file` avoids shell quoting issues, and `--transcript` writes a JSON transcript to disk. Human chat output is framed into a session card plus prompt and assistant panels, and markdown/code fences are rendered into a cleaner transcript instead of raw source text. Emojis are normalized to `0` in the console transcript. Use `/paste` to pull from the clipboard; if clipboard access is unavailable, the console falls back to manual paste mode and asks you to finish with `/end`. Use `/search <query>` to call the real `web.search` MCP tool and inject the results into the next turn. Tool calling is enabled by default in chat, and the console exposes the allowlisted MCP tool catalog to the model so it can call any approved tool. It also recognizes raw JSON tool-request envelopes like `{ "name": "...", "arguments": ... }` and executes them as tool calls instead of echoing them. Use `dotnet run --project .\tools\McpServer.AgentRouter.Tools -- ollama ls` to list the Ollama model catalog and `dotnet run --project .\tools\McpServer.AgentRouter.Tools -- ollama bench --report-dir <path>` to rank models and write `summary.json` plus `summary.csv` for comparisons across runs. Pass `--tools false` if you want plain single-turn completions and streaming output. On Windows, the wait indicator opens in a separate `McpServer Chat Status` window so it stays out of the transcript. Live redraw only kicks in on ANSI-capable terminals; otherwise the final formatted panel is emitted safely.
